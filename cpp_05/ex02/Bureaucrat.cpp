...
Bureaucrat::Bureaucrat(std::string Name, int Grade) : name(Name)
{
    ...
    {
        std::cout << Name << " Bureaucrat has been created" << std::endl;
        this->grade = Grade;
    }
    else if (Grade < 1)
    {
...
void    Bureaucrat::signForm(AForm& form) const
{
    if (this->grade <= form.get_grade_to_sign())
		std::cout << this->name << ", Bureaucrat signed " << form.getName() << " form" << std::endl;
	else
		std::cout << this->name << ", Bureaucrat couldn't sign " << form.getName()
                  << " form because its grade is lower than the required one" << std::endl;
}
...
void    Bureaucrat::executeForm(AForm const & form)
{
    ...
    {
        form.execute(*this);
        std::cout << name << " bureaucrat executed " << form.getName() << " Form\n";
    }
    catch(std::exception & e)
    {
        std::cout << name << " bureaucrat couldn't execute " << form.getName() << " Form because " << e.what() << std::endl;
    }
}
